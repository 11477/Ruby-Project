class DiscussionsController < ApplicationController
  ...
  def create
    ...
    @discussion.program=@program
    @user=current_user
    @discussion.user=@user

    respond_to do |format|
      if @discussion.save
        format.html { redirect_to @program, notice: "留言成功" }
        format.json { render :show, status: :created, location: @discussion }
      else
        format.html { render "programs/new", status: :unprocessable_entity }
  ...
  private
    ...
    def discussion_params
      params.require(:discussion).permit(:program_id, :content, :text)
    end
end
